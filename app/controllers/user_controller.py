...
def create_user(user: UserCreate, db: Session = Depends(get_db)):
    ...
    return StandardResponse(success=True, message="Usuário cadastrado com sucesso!", data={"id": result.id, "name": result.name})

@router.get("", response_model=StandardResponse)
def get_users(db: Session = Depends(get_db)):
    service = UserService(db)
    result = service.get_all_users()
    return StandardResponse(success=True, message="Dados do usuário carregados.", data=result)
